...
export default function Modal({ modalText, btnConfirmText = 'Confirm', btnRejectText = 'Reject', onConfirm, onReject }: IModalProps) {
  return (
    <div id="popup-modal" data-tabIndex="-1" className={style.wrapper}>
      <div className={style.div_outer}>
        <div className={style.div_inner}>
          <button type="button" className={style.button_times} data-modal-toggle="popup-modal">
...
const style = {
  wrapper: 'absolute overflow-y-auto overflow-x-hidden m-auto z-50 md:inset-0 flex justify-center h-modal md:h-full',
  div_outer: 'relative p-4 w-full max-w-md h-full md:h-auto',
